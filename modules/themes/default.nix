  ...
  config = mkIf (cfg.active != null) (mkMerge [
    {
      ...
      home.pointerCursor =
        let inherit (cfg.pointer) name package size;
        in {
          name = name;
          package = package;
          size = size;
      ...
      fonts =
        let inherit (cfg.fontConfig) packages emoji mono sans;
        in {
          packages = packages;
      ...
      hm.programs.vscode.extensions =
        let inherit (cfg.editor.vscode.extension) name publisher version hash;
    ...
    (mkIf (desktop.type == "x11") (mkMerge [
      {
        hm.xresources = {
          ...
          properties =
            let inherit (cfg.colors.main) bright normal types;
            in {
              "*.foreground" = "${types.fg}";
              "*.background" = "${types.bg}";
      ...
      (mkIf (cfg.onReload != { }) (
        let
          reloadTheme =
            let inherit (pkgs) stdenv writeScriptBin;
